 ...
 class myHashMap<K,V> {
     ...
     public V remove(K key) {
          ...
          */
          ...
          // check if the bucket's empty first
          if (bucket.get(index) == null){
            return null;
          }

          HashNode<K, V> cursor = bucket.get(index);
          V value = null; // the value that shall be returned
     ...
     public V replace(K key, V val) {
          ...
          */
          ...
          HashNode<K, V> cursor = bucket.get(index);
          V replacedValue = null; // the value that shall be returned

          // cursor traverses the entire bucket-list until it finds a hashNode matching with key.
          // if it does, replace the value and return it. else, return null and replace nothing
          while (cursor != null){ 
            if (cursor.key.equals(key)){
                replacedValue = cursor.value;
                cursor.value = val;
                return replacedValue;
            }
            cursor = cursor.next;
          }
         return null; // if target node isn't found, return null as nothing is removed
     }
     ...
     public boolean replace(K key, V oldVal, V newVal) {
         ...
         // first, see what value is mapped to given key. if it's null or doesn't match oldVal,
         // return false & replace nothing.
         V nodeValue = get(key);
         if (nodeValue == null || !nodeValue.equals(oldVal)) {
             return false;
         }  

         /* 
         * since a node with key mapped to oldVal exists, replace the node's value with newVal.
         * if replace returns no value, this method returns false. if replace returns a value,
         * this method returns true, as a node was successfully replaced.
         */
         return (replace(key, newVal) != null) ? true : false;
