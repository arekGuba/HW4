 ...
 class HashingProblems {
     ...
     public double getAverage(HashMap<Integer, Integer> map, int[] array) {
          ...
          */
           ...
           Double sum = 0.0; // sum of all values which have keys present in both map & array
           Double tracker = 0.0; // # of values which have keys present in both map & array

           /* 
           * checks if any element in array is present in map's keys. if matching ints are detected in both,
           * add the value for that key to our sum & increment the tracker.
           * aftewards, return sum / tracker for the average
           */
   ...
   public ArrayList<String> odd(HashMap<Integer, String> map) {
       ...
       ArrayList<String> result = new ArrayList<>(); // array which will house all values with odd keys
       ...
       if (map != null) { // null check
           Set<Integer> keys = map.keySet(); // set of all keys of map

           // for-each loop that checks every key in keys.
           // if a key is indeed odd, add the value for that key to result
           for (Integer i : keys) {
               if (i % 2 == 1) {
   ...
   public int twoSums(int[] numbers, int k) {
       ...
       // make a hashSet of all the integers in numbers subtracted by k in first loop.
       // if integers in the hashSet are present in numbers, increment tracker, then
       // return it after the second loop.

       if (numbers != null) {
           HashSet<Integer> subtractedNums = new HashSet<>();
           int count = 0;

           for (int i : numbers) {
               subtractedNums.add(i - k);
           }

           for (int i : numbers) {
               if (subtractedNums.contains(i)) {
                   count++;
               }
           }
           return count;
